...
RUN cp ${NDK_ROOT}/bin/llvm-ar ${NDK_ROOT}/bin/x86_64-linux-android-ar
RUN cp ${NDK_ROOT}/bin/llvm-ranlib ${NDK_ROOT}/bin/x86_64-linux-android-ranlib
RUN ARCH="-D__ANDROID_API__=23 android-x86_64" ./install_openssl.sh
...
# ENV LIBS -L$NDK_ROOT/lib64/clang/11.0.5/lib/linux/ /usr/local/openssl/lib/libcrypto.a -lclang_rt.builtins-x86_64-android
...
RUN ./otp_build configure --disable-jit --with-ssl=/usr/local/openssl/ --disable-dynamic-ssl-lib --without-javac --without-odbc --without-wx --without-debugger --without-observer --without-cdv --without-et --xcomp-conf=xcomp/erl-xcomp-x86_64-android.conf
...
RUN ./otp_build configure --disable-jit --with-ssl=/usr/local/openssl/ --disable-dynamic-ssl-lib --without-javac --without-odbc --without-wx --without-debugger --without-observer --without-cdv --without-et --xcomp-conf=xcomp/erl-xcomp-x86_64-android.conf LDFLAGS="-z global"
